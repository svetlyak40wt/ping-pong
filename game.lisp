;; To start the game, compile this buffer
;; and do
;; (CL-USER::START)
;; in the Listener REPL

(defpackage :ping-pong
  (:use #:cl)
  (:export #:start))
(in-package :ping-pong)
...
(capi:define-interface pong-game-rigt ()
  ...
  (:panes
   ...
   (left-player-score-panel capi:output-pane
                            ...
                            :title "отбитые мячи  " :title-position :left :title-args '(:foreground :black)
   ...
   (rigt-player-score-panel capi:output-pane
                            ...
                            :font (gp:make-font-description :family "Times" :size 16 :weight :bold :slant :roman)
                            :foreground :blue :background :grey
                            :title "  отбитые мячи" :title-position :right :title-args '(:foreground :black)
                            :title-font (gp:make-font-description :family "Times" :size 14 :weight :bold :slant :roman)
   ...
   (left-player-neprinyat-balls-panel capi:output-pane
                                      ...
                                      :font (gp:make-font-description :family "Times" :size 16 :weight :bold :slant :roman)
                                      :foreground :blue :background :grey
                                      :title "непринятые подачи  " :title-position :left :title-args '(:foreground :black)
                                      :title-font (gp:make-font-description :family "Times" :size 14 :weight :bold :slant :roman)
  ...
  (:layouts
   ...
   (rigt-area capi:pinboard-layout
              '() :background :deepskyblue2
                  ...
                  :input-model '(((:button-1 :motion) move-paddle-rigt))
                  :cursor :move))
                      ...
                      ((> x (- board-width (+ slug-width ball-size)))
                       (if (<= (nth-value 1 (capi:pinboard-pane-position right-paddle))
                           ...
                           (progn
                             ...
                             (setf x-movement (- x-movement))
                             (setf rigt-player-score (1+ rigt-player-score)) ; ОТБИТЫЕ мячи ПРАВЫМ игроком
                             (udar-raketkoy-sound)
                             )
                           (progn 
                             (when timer
                      ...
                      ((> slug-width x)
                       (if (<= (nth-value 1 (capi:pinboard-pane-position left-paddle))
                           ...
                           (progn
                             (setf y-movement (- (random 26) 12))
                             (setf x-movement (- x-movement))
                             (setf left-player-score (1+ left-player-score)) ; ОТБИТЫЕ мячи ЛЕВЫМ игроком
                             (udar-raketkoy-sound)
                             )
                           (progn 
                             (mp:unschedule-timer timer)
...
(defun vvod-names-of-players ()
  (setf left-player-name (capi:prompt-for-string "Введите имя и фамилию игрока на левой стороне"
                                                 :text "Rafael NADAL" )
        rigt-player-name (capi:prompt-for-string "Введите имя и фамилию игрока на правой стороне"
                                                 :text "Novak DJOKOVIC" )))

(defun rigt-player-score-counter (pane x y width height)
  (gp:draw-string pane
                  (princ-to-string (slot-value ekz-interf 'rigt-player-score))
                  (+ x 2) (+ y 19)))
      ...
      (if (and (= s4et4ik-rozygryshey 1) (= otbitye-poda4i-left otbitye-poda4i-iskhodn) )
          ...
          (if (and (= s4et4ik-rozygryshey 1) (> otbitye-poda4i-left otbitye-poda4i-iskhodn) )
              ...
              (if (and (> s4et4ik-rozygryshey 1) (> otbitye-poda4i-left otbitye-poda4i-iskhodn) )
                  ...
                  (if (and (> s4et4ik-rozygryshey 1)
                           (= otbitye-poda4i-left otbitye-poda4i-iskhodn))
                      (setq delta-neprinyat-podachi 1)))))))

(defun left-player-neprinyat-balls-counter (pane x y width height)
  (gp:draw-string pane
                  (princ-to-string (setq neprinyat-podachi
                                         (+ neprinyat-podachi delta-neprinyat-podachi)))
                  (+ x 2) (+ y 19)))
...
(defun aplodisments-before-game ()
  (setq pathname-aplodisments-before-game (media-path "aplodisments.wav")
...
(defun pervaya-poda4a-sound ()
  (setq pathname-pervaya-poda4a (media-path "pervaya-poda4a.wav")
...
(defun udar-raketkoy-sound ()
  (setq pathname-udar-raketkoy (media-path "udar-raketkoy.wav")
...
(defun left-aplodisments ()
  (setq pathname-left-aplodisments (media-path "left-player-aplodisments.wav")
...
(capi:define-interface pong-game-left ()
  ...
  (:panes
   ...
   (bot-border-line capi:rectangle       :x 0  :y (- game-height (* 0.5 slug-width))
                                         ...
                                         :graphics-args '(:foreground :white)  :filled t  )
   (new-game-button capi:push-button
                    :x 90 :y 68 :background :red  ; !!!
                    :text "Новый мяч" :mnemonic 4
  ...
  (:layouts
   ...
   (left-area capi:pinboard-layout
              '() :background :deepskyblue2
                  :visible-min-width (+ (* 0.25 game-width) 2)
                  :input-model '(((:button-1 :motion) move-paddle-left))
                  :cursor :move ))
  (:default-initargs :title "PING - PONG"
                     :confirm-destroy-callback 'interface-dead ))
                      ...
                      ((< x slug-width)
                       (if (<= (nth-value 1 (capi:pinboard-pane-position left-paddle))
                           ...
                           (progn
                             ...
                             (setf x-movement (- x-movement))
                             (setf left-player-score (1+ left-player-score)) ; ОТБИТЫЕ мячи LEVYM игроком
                             (udar-raketkoy-sound)
                             )
                           (progn 
                             (when timer
                      ...
                      ((> x (- board-width (+ slug-width ball-size)))
                       (if (<= (nth-value 1 (capi:pinboard-pane-position right-paddle))
                           ...
                           (progn
                             (setf y-movement (- (random 26) 12)) ; 13  6
                             (setf x-movement (- x-movement))
                             (setf rigt-player-score (1+ rigt-player-score)) ; ОТБИТЫЕ мячи PRAVYM игроком
                             (udar-raketkoy-sound)
                             )
                           (progn 
                             (mp:unschedule-timer timer)
...
(defun start ()
  (let ((active-player
        (capi:prompt-for-string
         "Если хотите играть за левую сторону, введите L, иначе - нажмите любую другую клавишу" :text "L" )))
       
   (if (string= active-player "L")
       (ping-pong-left)
     (ping-pong-rigt))))
...
(defun media-path (filename)
  "Возвращает путь относительно текущего файла."
  (when (uiop/pathname:pathname-equal (uiop:getcwd) "/")
    #+cocoa
    (let ((bundle (objc:invoke "NSBundle" "mainBundle")))
      (setf *resource-path*
            (concatenate 'string
                         (objc:invoke-into 'string bundle "resourcePath")
                         "/")))
    #-cocoa
    (error "Don't know how to locate wav files on systems other than OSX"))
  
  (unless *resource-path*
    (error "You need to compile the file to make relative paths work"))

  (merge-pathnames filename *resource-path*))
